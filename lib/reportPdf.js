...
const drawTable = (pdf, rows, y, columns) => {
  ...
  rows.forEach((row, rowIndex) => {
    const values = columns.map((column) => safeText(column.value(row)) || '—');
...
export function generateReportPdf(report) {
  ...
  const sections = [
    ...
    {
      title: 'C. Clients Needing Follow-Up / Solutions to Complaints',
  ...
  sections.forEach((section) => {
    ...
    y = drawTable(pdf, section.rows, y, [
      { label: 'NO.', width: 14, value: (_, index) => index + 1 },
      { label: 'CLIENT NAME', width: 54, value: (row) => row.clientName },
      { label: 'COMMENT OR FEEDBACK', width: CONTENT_WIDTH - 68, value: (row) => row.comment || row.contactMethod || '—' },
    ]);
  });
