...
const collapseCharacterSpacedWords = (text) => text.split(/(\s{2,})/).map((segment) => {
  const trimmed = segment.trim();
  const characters = trimmed.split(/\s+/);
  const isSpacedCharacter = (character) => /^[A-Za-z][.,!?;:'"“”’)]*$/.test(character);
  if (characters.length >= 2 && characters.every(isSpacedCharacter)) {
    const leadingWhitespace = segment.match(/^\s*/)?.[0] || '';
    const trailingWhitespace = segment.match(/\s*$/)?.[0] || '';
    return `${leadingWhitespace}${characters.join('').replace(/([A-Za-z])([.,!?;:'"“”’)]*)$/, '$1$2')}${trailingWhitespace}`;
  }
  return segment;
}).join('');

const safeText = (value) => Array.from(String(value ?? '').normalize('NFKC'))
  .map((character) => {
    const codePoint = character.codePointAt(0);
    if (codePoint <= 0xff || character === '\n' || character === '\t') return character;
    return PDF_CHAR_REPLACEMENTS[character] || '';
  })
  .join('')
  .replace(/\r\n?/g, '\n')
  .split('\n')
  .map((line) => collapseCharacterSpacedWords(line).replace(/[ \t]+/g, ' ').trim())
  .join('\n')
  .trim();

const resetTextSpacing = (pdf) => {
  ...
  const drawRowSegment = (wrapped, rowIndex, lineStart, lineCount, segmentHeight) => {
    ...
    wrapped.forEach((lines, index) => {
      ...
      if (segmentLines.length) {
        pdf.text(segmentLines, x + cellPaddingX, y + cellPaddingY, {
          baseline: 'top',
          maxWidth: Math.max(scaledColumns[index].width - cellPaddingX * 2, 8),
        });
      }
...
const drawNotes = (pdf, notes, y, contentWidth) => {
  const lineHeight = 4.5;
  const padding = 5;
  const bottomMargin = 18;
  const lines = pdf.splitTextToSize(notes, contentWidth - padding * 2);
  let lineStart = 0;

  while (lineStart < lines.length) {
    const { height: pageHeight } = pageMetrics(pdf);
    y = addPageIfNeeded(pdf, y, lineHeight + padding * 2);
    const availableHeight = pageHeight - bottomMargin - y;
    const linesThatFit = Math.max(1, Math.floor((availableHeight - padding * 2) / lineHeight));
    const lineCount = Math.min(lines.length - lineStart, linesThatFit);
    const boxHeight = Math.max(16, lineCount * lineHeight + padding * 2);
    ...
    pdf.setFontSize(9);
    pdf.setTextColor(51, 65, 85);
    pdf.text(lines.slice(lineStart, lineStart + lineCount), MARGIN + padding, y + padding, { baseline: 'top' });

    y += boxHeight + 4;
    lineStart += lineCount;
    if (lineStart < lines.length) y = addPageIfNeeded(pdf, y, lineHeight + padding * 2);
  }
...
export function generateReportPdf(report) {
  ...
  pdf.text('Feedback, Birthdays & Calls Report', MARGIN, 27);
  pdf.setFontSize(9.5);
  pdf.setTextColor(51, 65, 85);
  ...
  if (safeText(report?.notes)) {
    ...
    y = drawNotes(pdf, safeText(report.notes), y, contentWidth);
  }
