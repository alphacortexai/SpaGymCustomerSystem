...
/**
 * Move an active gym membership into a selected spa membership type.
 * The source record is retained as history and marked as transferred while
 * the new spa record keeps the source expiry date and links both records.
 */
export async function transferGymEnrollmentToSpa(
  ...
  try {
    if (!gymEnrollmentId || !spaMembershipTypeId || !branch?.trim()) {
      return { success: false, error: 'Gym membership, spa membership type, and branch are required.' };
    }

    const gymEnrollmentRef = doc(db, ENROLLMENTS_COLLECTION, gymEnrollmentId);
    const spaTypeRef = doc(db, SPA_MEMBERSHIP_TYPES_COLLECTION, spaMembershipTypeId);
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const spaTypeSnapshot = await transaction.get(spaTypeRef);

      if (!gymSnapshot.exists()) throw new Error('Gym membership not found.');
      if (!spaTypeSnapshot.exists()) throw new Error('Spa membership type not found.');

      const source = gymSnapshot.data();
      const targetType = spaTypeSnapshot.data();
      const activeSpaQuery = query(
        collection(db, SPA_ENROLLMENTS_COLLECTION),
        where('clientId', '==', source.clientId),
        where('status', '==', 'active'),
      );
      const activeSpaSnapshot = await transaction.get(activeSpaQuery);
      const now = new Date();
      const sourceExpiry = source.expiryDate?.toDate?.();
      const hasActiveSpaMembership = activeSpaSnapshot.docs.some((spaDoc) => {
        const spaData = spaDoc.data();
        const spaExpiry = spaData.expiryDate?.toDate?.();
        return spaData.status === 'active' && spaExpiry && spaExpiry > now;
      });

      if (source.status !== 'active' || !sourceExpiry || sourceExpiry <= now) {
        throw new Error('Only active, non-expired gym memberships can be transferred.');
      }
      if (hasActiveSpaMembership) {
        throw new Error(`Client already has an active spa membership. Cancel it before transferring this membership.`);
      }

      const remainingDays = Math.max(1, Math.ceil((sourceExpiry.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)));
      const isReducingBalance = Boolean(targetType.isReducingBalance);
      const requestedBalance = transferBalance === null || transferBalance === ''
        ? (source.isReducingBalance ? source.balance : targetType.price)
        : Number(transferBalance);
      const finalPrice = isReducingBalance ? requestedBalance : Number(targetType.price);

      if (!Number.isFinite(finalPrice) || finalPrice < 0) {
        throw new Error('The transfer amount must be a valid non-negative number.');
      }
      ...
      targetMembershipName = targetType.type || '';

      transaction.set(spaEnrollmentRef, {
        clientId: source.clientId,
        clientName: source.clientName,
        membershipTypeId: spaMembershipTypeId,
        membershipType: targetType.type,
        price: finalPrice,
        currency: targetType.currency || source.currency || 'USD',
        ...
        entitlements: isReducingBalance ? [] : (targetType.entitlements || []),
        isReducingBalance,
        balance: isReducingBalance ? finalPrice : 0,
        redeemedEntitlements: [],
        treatments: [],
      ...
      transaction.update(gymEnrollmentRef, {
        ...
        transferredTo: 'spa',
        transferTargetMembershipType: targetType.type || '',
        updatedAt: Timestamp.now(),
      });
    });
    ...
    if (currentUser) {
      ...
      await logAction({
        ...
        details: `Transferred ${sourceClientName}'s gym membership to spa membership ${targetMembershipName}`,
      });
    }
