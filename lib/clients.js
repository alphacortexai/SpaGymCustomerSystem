...
/**
 * Get client counts by branch (optimized for badges)
 * Note: Firestore client SDK doesn't support aggregation queries,
 * so we fetch documents but only use branch field for counting.
 * For better performance with large datasets, consider using Cloud Functions
 * with Admin SDK for true aggregation queries.
 */
export async function getClientCountsByBranch() {
  try {
    const clientsRef = collection(db, CLIENTS_COLLECTION);
...
/**
 * Get birthday counts by branch (optimized for badges)
 * Only checks birthMonth and birthDay fields, not full date parsing
 * Note: For better performance with large datasets, consider using Cloud Functions
 * with Admin SDK for true aggregation queries.
 */
export async function getBirthdayCountsByBranch() {
  try {
    const today = new Date();
    ...
    snapshot.forEach((doc) => {
      ...
      // Check if birthday matches today using stored month/day (fast)
      if (data.birthMonth === todayMonth && data.birthDay === todayDate) {
        if (branch && branch.trim()) {
          counts[branch] = (counts[branch] || 0) + 1;
        }
