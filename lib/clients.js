...
export async function updateBirthdayCall(clientId, callData, currentUser = null) {
  ...
  try {
    const clientRef = doc(db, CLIENTS_COLLECTION, clientId);
    const isClearing = !callData?.calledById;
    const updateData = {
      birthdayCallStatus: isClearing ? 'not_called' : 'called',
      birthdayCalledById: isClearing ? null : callData.calledById,
    ...
    if (currentUser) {
      ...
      await logAction({
        ...
        userName: currentUser.displayName || currentUser.email,
        userEmail: currentUser.email,
        action: isClearing ? 'EDIT' : 'BIRTHDAY_CALL',
        targetType: 'CLIENT',
        targetId: clientId,
        targetName: callData.clientName || 'Birthday client',
        details: isClearing
          ? 'Cleared birthday call verification'
          : `Marked birthday call as completed by ${callData.calledByName || 'staff member'}`,
      });
    }
