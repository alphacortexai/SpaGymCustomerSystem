...
export function getClientSearchHints(clients = [], searchTerm = '', branch = null, limit = 50) {
  const queryText = normalizeClientSearchText(searchTerm);
  const compactQuery = queryText.replace(/ /g, '');
  const queryPhoneVariants = getClientPhoneSearchVariants(searchTerm);
  const matches = filterClientsBySearch(clients, searchTerm, branch);

  return matches
    .map((client) => {
      const nameText = normalizeClientSearchText(client?.name || '');
      const compactName = nameText.replace(/ /g, '');
      const phoneVariants = getClientPhoneSearchVariants(client?.phoneNumber || '');
      const exactName = nameText === queryText;
      const startsName = nameText.startsWith(queryText) || compactName.startsWith(compactQuery);
      const phoneMatch = queryPhoneVariants.some((queryPhone) => phoneVariants.some((phone) => phone.startsWith(queryPhone) || phone.includes(queryPhone)));
      const score = exactName ? 0 : startsName ? 1 : phoneMatch ? 2 : 3;
      return { client, score };
    })
