...
export async function syncUser(firebaseUser) {
  ...
  if (!userSnap.exists()) {
    // Bootstrap admin access is opt-in through deployment configuration; all other new accounts remain pending.
    const isDefaultAdmin = Boolean(DEFAULT_ADMIN_EMAIL) && firebaseUser.email === DEFAULT_ADMIN_EMAIL;
    const userData = {
 ...
 */
export async function updateUserBranches(uid, assignedBranches) {
  try {
    const userRef = doc(db, USERS_COLLECTION, uid);
