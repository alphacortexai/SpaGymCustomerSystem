...
export default function SpaMembershipTypeManager() {
  ...
  const loadTypes = async () => {
    setLoading(true);
    const data = await getMembershipTypes(true);
    setTypes(data);
    setLoading(false);
  };
  ...
  if (loading && types.length === 0) return <div className="text-center py-10">Loading spa types...</div>;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Manage Spa Membership Types</h2>
        <button onClick={loadTypes} className="text-sm text-indigo-600 hover:underline">Refresh</button>
      </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {type.isReducingBalance && (
                      <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 text-[10px] font-bold rounded-md uppercase">Reducing Balance</span>
                    )}
                  </div>
