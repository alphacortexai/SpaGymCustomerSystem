...
export default function MembershipList() {
  ...
  if (loading && enrollments.length === 0) {
    return <LoadingState title="Loading memberships" description="Preparing membership records." />;
  }
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          <div className="relative w-full sm:flex-1 md:w-64">
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-sm outline-none focus:ring-2 focus:ring-blue-500"
            />
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
              <button 
                ...
                className={`p-1.5 rounded-lg transition-all ${viewMode === 'table' ? 'bg-white dark:bg-slate-700 shadow-sm text-blue-600' : 'text-slate-500'}`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>
              </button>
              <button 
                onClick={() => setViewMode('card')}
                className={`p-1.5 rounded-lg transition-all ${viewMode === 'card' ? 'bg-white dark:bg-slate-700 shadow-sm text-blue-600' : 'text-slate-500'}`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
              </button>
            </div>

            <button onClick={loadEnrollments} className="text-sm text-blue-600 hover:underline whitespace-nowrap">Refresh</button>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Remaining</th>
                ...
                <tr key={enrollment.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                    {enrollment.isReducingBalance
                      ? ((enrollment.currency || 'UGX') === 'UGX' ? 'UGX ' : '$') + Number(enrollment.balance || 0).toLocaleString()
                      : '—'}
                  </td>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {paginatedEnrollments.map((enrollment) => (
            <div key={enrollment.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
              ...
              <div className="space-y-2 mb-4">
                ...
                {enrollment.isReducingBalance && (
                  <div className="flex items-center text-sm text-blue-600 dark:text-blue-400 font-medium">
                    ...
                    Remaining: {((enrollment.currency || 'UGX') === 'UGX' ? 'UGX ' : '$') + Number(enrollment.balance || 0).toLocaleString()}
                  </div>
                )}
              ...
              <button 
                onClick={() => setSelectedEnrollment(enrollment)}
                className="w-full py-2 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-bold hover:bg-blue-50 hover:text-blue-600 transition-all"
              >
                View Details
              </button>
            </div>
