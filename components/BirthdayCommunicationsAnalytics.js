  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="50% offer redeemed" value={analytics.redeemed} detail={`${analytics.redeemed} of ${analytics.total} birthdays (${analytics.redemptionRate}) · ${analytics.redeemedContacted} of ${analytics.contacted} contacted (${analytics.contactedRedemptionRate})`} accent="violet" />
      </div>
      ...
      <section className="rounded-2xl border border-violet-100 bg-gradient-to-r from-violet-50/70 via-white to-pink-50/60 p-5 shadow-sm dark:border-violet-900/40 dark:from-violet-950/20 dark:via-slate-900 dark:to-pink-950/20">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="font-black text-slate-900 dark:text-white">Birthday offer performance</h3><p className="mt-1 text-xs font-medium text-slate-500">50% birthday offer redemptions compared with the birthday population and completed outreach.</p></div><div className="rounded-xl bg-white/80 px-3 py-2 text-right shadow-sm dark:bg-slate-900/70"><div className="text-lg font-black text-violet-700 dark:text-violet-200">{analytics.redeemed}</div><div className="text-[10px] font-black uppercase tracking-wider text-slate-400">Redeemed offer</div></div></div>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {[{ label: 'Birthday population', value: analytics.total, denominator: analytics.total, percent: 100, color: 'bg-blue-400' }, { label: 'Contacted', value: analytics.contacted, denominator: analytics.total, percent: analytics.total ? (analytics.contacted / analytics.total) * 100 : 0, color: 'bg-pink-400' }, { label: 'Redeemed offer (contacted or uncontacted)', value: analytics.redeemed, denominator: analytics.total, percent: analytics.total ? (analytics.redeemed / analytics.total) * 100 : 0, color: 'bg-violet-500' }].map((item) => <div key={item.label}><div className="mb-1.5 flex items-center justify-between text-xs font-bold text-slate-600 dark:text-slate-300"><span>{item.label}</span><span>{item.value} <span className="font-medium text-slate-400">({formatPercent(item.value, item.denominator)})</span></span></div><div className="h-2.5 overflow-hidden rounded-full bg-white/80 dark:bg-slate-800"><div className={`h-full rounded-full ${item.color}`} style={{ width: `${Math.min(item.percent, 100)}%` }} /></div></div>)}
        </div>
      </section>
